...
class AskAnswerLikeDao(BaseDao):
    db_name = "doctor"
    table_name = "ask_answer_like"
    ...
    @classmethod
    def insert(cls, ask_id, answer_id, user_id):
        """
        ...
        :return:
        """
        sql = "insert into {db}.{tbl}(ask_id, answer_id, user_id) values ({ask_id}, {answer_id}, {user_id}) " \
    ...
    @classmethod
    def get_by_answer_user(cls, answer_id, user_id):
        """
        获取用户对此回答的点赞信息
        :param answer_id:
        :param user_id:
        :return:
        """
        sql = "select 1 from {db}.{tbl} where del_flag=0 and answer_id={answer_id} and user_id={user_id}". \
